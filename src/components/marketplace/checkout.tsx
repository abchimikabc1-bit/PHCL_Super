// components/marketplace/checkout.tsx
'use client';

import React, { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';

// Sarafu na payment methods kutoka kwenye currencies ya lib
import { 
  CURRENCIES, 
  type CurrencyCode, 
  PAYMENT_METHODS, 
...
// Kazi za ubadilishaji kutoka kwenye currency ya components
import { convertCurrency, formatCurrency } from '@/components/currency';

// Kazi za bidhaa kutoka kwenye marketplace-products ya lib
...
export default function Checkout() {
  const [cartItems] = useState<CartItem[]>([
    { product: MARKETPLACE_PRODUCTS[0], quantity: 1 },
    { product: MARKETPLACE_PRODUCTS[6] || MARKETPLACE_PRODUCTS[1], quantity: 2 },
  ]);
          ...
          {orderSuccess && (
            ...
            >
              ...
              <p className="text-slate-300 text-sm max-w-lg mx-auto">
                Asante <strong className="text-white">{shippingInfo.fullName}</strong>! Malipo ya <strong className="text-emerald-400">{formatCurrency(totalConverted, selectedCurrency)}</strong> kupitia <strong className="text-white">{selectedPayment.name}</strong> yamethibitishwa. Tutawasiliana nawe kupitia <strong className="text-white">{shippingInfo.phone}</strong>.
              </p>
              <button
                type="button"
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-slate-900 border border-slate-800/90 rounded-3xl p-6 space-y-5 sticky top-6">
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-slate-950 font-black py-4 rounded-2xl shadow-lg transition transform active:scale-[0.99] flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
              >
                {isProcessing ? (
                  <>
