...
const READ_BUTTON_BY_LANG: Record<VoiceLanguage, string> = {
  sw: 'Soma Salio kwa Sauti', en: 'Read Balance Aloud', zh: '朗读余额', fr: 'Lire le solde à voix haute',
};

const LAST_READ_LABEL_BY_LANG: Record<VoiceLanguage, string> = {
  sw: 'Imesomwa', en: 'Read out', zh: '已朗读', fr: 'Lu à voix haute',
};

const VOICE_LANGUAGE_TITLE_BY_LANG: Record<VoiceLanguage, string> = {
  sw: 'Lugha ya Sauti', en: 'Voice Language', zh: '语音语言', fr: 'Langue vocale',
};
...
const INPUT_TITLE_BY_LANG: Record<VoiceLanguage, string> = {
  sw: 'Andika Kiasi cha PI', en: 'Enter PI Amount', zh: '输入 PI 数量', fr: 'Saisir le montant PI',
};

const INPUT_PLACEHOLDER_BY_LANG: Record<VoiceLanguage, string> = {
  sw: 'Mfano: 2.5', en: 'Example: 2.5', zh: '例如：2.5', fr: 'Exemple : 2.5',
};

const READ_ESTIMATE_BUTTON_BY_LANG: Record<VoiceLanguage, string> = {
  sw: 'Soma Makadirio ya Thamani', en: 'Read Estimated Value', zh: '朗读估算价值', fr: 'Lire la valeur estimée',
};

const INVALID_AMOUNT_BY_LANG: Record<VoiceLanguage, string> = {
  sw: 'Tafadhali andika kiasi sahihi cha PI.', en: 'Please enter a valid PI amount.',
...
const MESSAGE_BY_LANG = (language: VoiceLanguage, balancePi: string, gcvUsd: string) => {
  ...
  if (language === 'zh') {
    return `钱包余额为 ${balancePi} Pi。按照 GCV，1 个 Pi 等于 ${gcvUsd} 美元。目前我们的平台即将完成，请稍候。`;
  }
  if (language === 'fr') {
    return `Le solde du portefeuille est de ${balancePi} Pi. Un Pi équivaut à ${gcvUsd} dollars américains au taux GCV. Pour l'instant, notre plateforme est presque terminée; veuillez patienter un peu.`;
  }
  return `Salio la wallet ni ${balancePi} Pi. Pi moja ni sawa na dola ${gcvUsd} kwa kiwango cha GCV. Kwa sasa mfumo wetu unakaribia kukamilika; tafadhali subiri kidogo.`;
};
...
export default function WalletVoiceAssist({ balancePi, gcvUsd }: { balancePi: string; gcvUsd: string }) {
  ...
  const speak = () => {
    ...
    const familyVoice = voices.find((v) => v.lang.toLowerCase().startsWith(langCode.slice(0, 2).toLowerCase()));
  ...
  const speakEstimatedValue = () => {
    const amountPi = Number(typedPiAmount.trim().replace(',', '.'));
  ...
  const handleLanguageSelect = (nextLanguage: VoiceLanguage) => {
    ...
    setLastRead('');
    if (typeof window !== 'undefined') window.localStorage.setItem(VOICE_LANGUAGE_STORAGE_KEY, nextLanguage);
  };
  ...
  return (
    <div className="mt-4 rounded-xl border border-cyan-200/25 bg-white/5 p-4">
      <div className="mb-3 rounded-lg border border-cyan-200/25 bg-slate-900/50 p-3">
        <p className="mb-2 text-[11px] font-bold uppercase tracking-[0.16em] text-cyan-100/90">{VOICE_LANGUAGE_TITLE_BY_LANG[language]}</p>
        <div className="flex flex-wrap gap-2">
          {VOICE_LANGUAGE_LABELS.map((option) => (
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-xs font-bold transition ${language === option.code ? 'bg-cyan-300 text-slate-900 ring-2 ring-cyan-100/70' : 'border border-cyan-200/40 bg-cyan-500/10 text-cyan-50 hover:bg-cyan-500/20'}`}
            >
              {option.label}
            </button>
          ))}
      ...
      <button type="button" onClick={speak} className="rounded-lg border border-cyan-300/45 bg-cyan-500/20 px-3 py-2 text-xs font-bold text-cyan-50 hover:bg-cyan-500/30">
      ...
      {lastRead && <p className="mt-3 text-xs text-cyan-100/90">{LAST_READ_LABEL_BY_LANG[language]}: {lastRead}</p>}
    </div>
  );
}
