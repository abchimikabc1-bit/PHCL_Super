...
export default function AdminCurrenciesPage() {
  ...
  useEffect(() => {
    if (!snapshot) return;
    setConfig(snapshot.currencyConfig ?? getAdminCurrencyConfig());
  ...
  const persist = () => {
    ...
    void (async () => {
      ...
      if (!response.ok || !payload?.success) {
        toast.error(payload?.error || 'Failed to save currency configuration');
      ...
      await refreshCommerceClientCache();
      setConfig(payload.snapshot?.currencyConfig ?? config);
      setAuditEvents(payload.snapshot?.currencyAudit ?? getAdminCurrencyAudit());
