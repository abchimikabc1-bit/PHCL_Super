...
function normalizeMsisdn(
  ...
  /*
   * Accept valid Tanzanian mobile numbers without relying
   * on the original operator prefix. Mobile-number
   * portability may move a number between providers.
   */
  if (
    !/^255[67][0-9]{8}$/.test(
      normalized,
...
> {
  ...
  return {
    ...
    customerAction: {
      ...
      message:
        'Approve the HaloPesa sandbox payment prompt on your phone.',
...
export const halopesaSandboxAdapter:
  PaymentProviderAdapter =
    Object.freeze({
      providerCode:
        PROVIDER_CODE,
