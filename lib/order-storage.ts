...
export const ORDER_STORAGE_KEY = 'phcl_orders';
...
let attemptedOrderHydration = false;
...
const normalizeOrder = (raw: unknown): { order: StoredOrder | null; reason?: string } => {
  ...
  if (row.audit && typeof row.audit === 'object') {
    ...
    if (audit.cancellation && typeof audit.cancellation === 'object') {
      const rawCancellation = audit.cancellation as {
        cancelledAt?: unknown;
        reason?: unknown;
        refunded?: unknown;
        refundedAt?: unknown;
      ...
      const cancelledAt = trimText(rawCancellation.cancelledAt, 60);
      if (cancelledAt) {
        const refundedAt = trimText(rawCancellation.refundedAt, 60) || undefined;
        const reason = trimText(rawCancellation.reason, 180) || undefined;
        const refundAmountRaw = Number(rawCancellation.refundAmount);
        const refundAmount = Number.isFinite(refundAmountRaw)
          ? Math.max(0, Number(refundAmountRaw.toFixed(8)))
          : undefined;
        const refundCurrency = isCurrency(rawCancellation.refundCurrency)
          ? rawCancellation.refundCurrency
          : undefined;
        const refundTransactionId = trimText(rawCancellation.refundTransactionId, 80) || undefined;

        cancellation = {
          cancelledAt,
          reason,
          refunded: rawCancellation.refunded === true,
          refundedAt,
          refundAmount,
          refundCurrency,
          refundTransactionId,
...
export const sanitizeStoredOrder = (raw: unknown): StoredOrder | null => normalizeOrder(raw).order;

const hydrateOrdersFromServer = (): void => {
  if (attemptedOrderHydration || !canUseStorage()) return;
  attemptedOrderHydration = true;

  void hydrateCommerceStateFromServer().then((snapshot) => {
    if (!snapshot || !Array.isArray(snapshot.orders) || snapshot.orders.length === 0) return;

    const next = snapshot.orders
      .map((entry) => sanitizeStoredOrder(entry))
      .filter((entry): entry is StoredOrder => !!entry)
      .slice(0, MAX_ORDERS);
...
export const getOrders = (): StoredOrder[] => {
  ...
  try {
    ...
    if (report.validOrders.length === 0) {
      hydrateOrdersFromServer();
    }
...
export const saveOrder = (order: StoredOrder): StoredOrder[] => {
  ...
  window.localStorage.setItem(ORDER_STORAGE_KEY, JSON.stringify(next));
  window.dispatchEvent(new Event(ORDERS_UPDATED_EVENT));
  syncCommerceStateToServer({ orders: next });
  return next;
};
...
export const updateOrderById = (
  ...
  const nextOrder = updater(current[index]);
  const normalized = normalizeOrder(nextOrder).order;
