...
export const CART_STORAGE_KEY = 'phcl_cart_items';
...
let attemptedCartHydration = false;
...
export const sanitizeCartItems = (items: unknown): CartStorageItem[] => normalizeCartItems(items);

const hydrateCartFromServer = (): void => {
  if (attemptedCartHydration || !canUseStorage()) return;
  attemptedCartHydration = true;

  void hydrateCommerceStateFromServer().then((snapshot) => {
    if (!snapshot || !Array.isArray(snapshot.cartItems) || snapshot.cartItems.length === 0) return;
    const next = sanitizeCartItems(snapshot.cartItems).slice(0, MAX_CART_ITEMS);
...
export const getCartItems = (): CartStorageItem[] => {
  ...
  try {
    ...
    const next = normalizeCartItems(parsed);
    if (next.length === 0) {
      hydrateCartFromServer();
    }
    return next;
...
export const setCartItems = (items: CartStorageItem[]): void => {
  ...
  window.dispatchEvent(new Event(CART_UPDATED_EVENT));
  syncCommerceStateToServer({ cartItems: sanitized });
};
