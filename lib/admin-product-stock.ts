...
export function createDefaultProductStockConfig(): ProductStockConfig {
  return createDefaultConfig();
}

export function sanitizeProductStockConfig(config?: unknown): ProductStockConfig | null {
  if (!config || typeof config !== 'object') return null;
  return normalizeProductConfig(config as ProductStockConfig);
}

export function sanitizeProductStockAudit(entries: unknown): ProductStockAuditEntry[] {
  if (!Array.isArray(entries)) return [];
  return entries
    .filter((entry): entry is ProductStockAuditEntry => {
      if (!entry || typeof entry !== 'object') return false;
      const row = entry as ProductStockAuditEntry;
      return typeof row.timestamp === 'string' && typeof row.actor === 'string' && typeof row.action === 'string';
    })
    .slice(-MAX_AUDIT_ENTRIES);
}

function hydrateStockFromServer(): void {
  if (attemptedStockHydration || !canUseStorage()) return;
  attemptedStockHydration = true;

  void fetch('/api/admin/stock', { cache: 'no-store' })
    .then(async (response) => {
      if (!response.ok) return null;
      return (await response.json()) as { success?: boolean; config?: ProductStockConfig; audit?: ProductStockAuditEntry[] };
    })
    .then((payload) => {
      if (!payload?.success) return;
      if (payload.config) {
        window.localStorage.setItem(STORAGE_KEY, JSON.stringify(normalizeProductConfig(payload.config)));
      }
      if (payload.audit) {
        window.localStorage.setItem(AUDIT_KEY, JSON.stringify(sanitizeProductStockAudit(payload.audit)));
      }
    })
    .catch(() => {
...
export function getProductStockAudit(): ProductStockAuditEntry[] {
  ...
  try {
    const stored = storage.getItem(AUDIT_KEY);
    if (stored) {
      return sanitizeProductStockAudit(JSON.parse(stored));
    }
  } catch (err) {
...
export function revertProductStockPurchase(
  items: ProductStockPurchaseItem[],
  actor: string = 'system_refund'
): { success: boolean; reason?: string } {
  const storage = getStorage();
  if (!storage) {
    return { success: false, reason: 'Storage is unavailable' };
  ...
  try {
    ...
    console.error('Error reverting stock purchase:', err);
    return { success: false, reason: 'Could not restore stock after cancellation' };
  }
}
