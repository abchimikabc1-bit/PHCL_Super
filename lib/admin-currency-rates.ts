...
let attemptedCurrencyHydration = false;
...
export const sanitizeAdminCurrencyConfig = (raw: unknown): AdminCurrencyConfig | null => {
  if (!raw || typeof raw !== 'object') return null;
...
export const sanitizeAdminCurrencyAudit = (raw: unknown): AdminCurrencyAuditEntry[] => {
...
const hydrateCurrencyFromServer = (): void => {
  if (attemptedCurrencyHydration || !canUseStorage()) return;
  attemptedCurrencyHydration = true;

  void hydrateCommerceStateFromServer().then((snapshot) => {
    if (!snapshot) return;
    const config = sanitizeAdminCurrencyConfig(snapshot.currencyConfig);
    const audit = sanitizeAdminCurrencyAudit(snapshot.currencyAudit);
    if (config) {
