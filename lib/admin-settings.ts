...
let attemptedSettingsHydration = false;
...
export const getDefaultAdminSettings = (): AdminSystemSettings => ({ ...DEFAULT_SETTINGS });
...
export const sanitizeAdminSettings = (raw: unknown): AdminSystemSettings | null => {
...
export const sanitizeAdminSettingsAudit = (raw: unknown): AdminSettingsAuditEntry[] => {
...
const hydrateSettingsFromServer = (): void => {
  if (attemptedSettingsHydration || !canUseStorage()) return;
  attemptedSettingsHydration = true;

  void hydrateCommerceStateFromServer().then((snapshot) => {
    if (!snapshot) return;
    const settings = sanitizeAdminSettings(snapshot.adminSettings);
    const audit = sanitizeAdminSettingsAudit(snapshot.adminSettingsAudit);
    if (settings) {
      window.localStorage.setItem(ADMIN_SETTINGS_KEY, JSON.stringify(settings));
    }
    if (audit.length > 0) {
      window.localStorage.setItem(ADMIN_SETTINGS_AUDIT_KEY, JSON.stringify(audit));
    }
  });
};
...
export const saveAdminSettings = (settings: AdminSystemSettings, actor = 'admin'): AdminSystemSettings => {
  ...
  if (canUseStorage()) {
    ...
    syncCommerceStateToServer({
      ...
      adminSettingsAudit: readAuditEntries(),
    });
  }
