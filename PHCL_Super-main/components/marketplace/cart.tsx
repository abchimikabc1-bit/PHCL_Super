...
import { getProductPrice, getCurrencyColor, formatCurrencyAmount, PI_GCV_USD } from './currency-utils';
...
export function ShoppingCart({
  ...
  return (
    <div className={`${darkMode ? 'bg-slate-900 border border-violet-400/20' : 'bg-gradient-to-br from-indigo-50 via-white to-cyan-50 border border-violet-200'} rounded-xl p-6 mb-6 shadow-lg`}>
        ...
        {cart.map((item, index) => (
          ...
          >
            <div className="flex items-center gap-3 flex-1">
              ...
              <div className="flex-1">
                ...
                <p className={getCurrencyColor(cartCurrency)}>
                  {formatCurrencyAmount(cartCurrency, getProductPrice(item, cartCurrency))}
                </p>
              </div>
            </div>
            ...
            <p className={`font-bold text-lg ml-4 ${getCurrencyColor(cartCurrency)} w-24 text-right`}>
              {formatCurrencyAmount(cartCurrency, getProductPrice(item, cartCurrency) * item.quantity).replace(/^\S+\s/, '')}
            </p>
          </div>
        ))}
      </div>
      <div className={`border-t-2 pt-4 ${darkMode ? 'border-gray-600' : 'border-gray-300'}`}>
        <div className="flex justify-between items-center mb-4">
          ...
          <p className={`text-3xl font-bold ${getCurrencyColor(cartCurrency)}`}>
            {formatCurrencyAmount(cartCurrency, onGetTotal())}
          </p>
        </div>
        <p className={`text-xs mb-4 ${darkMode ? 'text-violet-200' : 'text-violet-700'}`}>
          Pi conversion uses GCV rate: 1 PI = ${PI_GCV_USD.toLocaleString('en-US')}
        </p>
